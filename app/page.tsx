...
export default function Home() {
  return <main>
    ...
    <section className="content-section proof-entry-section"><div className="section-head"><div><p className="section-kicker">OPEN PROJECT · AUDITABLE CLAIMS</p><h2>产品可以体验，研究也必须经得起检查</h2></div><p>为研究、求职和开源协作者提供一条独立评估路径：问题、贡献、实现、测试、局限与下一步分别说明。</p></div><div className="proof-entry-grid"><article><code>RUNNING PRODUCT</code><h3>从交互检查产品主张</h3><p>发现、调用、派生、组合和范围化复核已经进入可运行的多页面原型。</p><Link href="/explore">进入产品 →</Link></article><article><code>OPEN SPECIFICATION</code><h3>从Schema检查技术主张</h3><p>三个不同模块化程度的合成样本通过机器结构校验，同时明确“不证明法律正确”。</p><a href="https://github.com/dreamlee666/jurissemble/tree/main/spec" target="_blank" rel="noreferrer">查看公开规范 ↗</a></article><article className="accent"><code>PROJECT DOSSIER</code><h3>五分钟完成严肃评估</h3><p>查看FLC+D架构、可核验构件、复现命令、作者责任和当前未完成的验证。</p><Link href="/dossier">打开项目档案 →</Link></article></div></section>
